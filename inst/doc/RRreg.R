...
## -----------------------------------------------------------------------------
warner <- RRuni(response = response, 
                data = data.W, 
                model = "Warner", 
                p = .2, 
                MLest = T)
...
lin2 <- RRlin(formula = cov ~ response + pred, 
              data = data.W, 
              models = "Warner", 
              p.list = list(.2), 
              bs.n = 0, 
...
lin3 <- RRlin(cov ~ response + response2,
              data = data.W, 
...
fit <- RRlog(response ~ RR.design * z.age, 
             model = "Warner", 
             p = 2/12, 
             group = group, 
             LR.test = TRUE, 
...
linmod <- RRlin(cov ~ RR1$response + RR2$response, 
                models = c("SLD","SLD"), 
                p.list = list(c(.2, .8), c(.3, .7)), 
                group = group, 
                fit.n = 1)
