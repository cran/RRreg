## -----------------------------------------------------------------------------
library(RRreg)
data.W <- RRgen(n=1000, pi.true=.3, model="Warner", p=.2, 
                complyRates = c(1, 1), sysBias = c(0,0))
head(data.W)

## -----------------------------------------------------------------------------
warner <- RRuni(response=response, data=data.W, 
                model="Warner", p=.2, MLest=T)
summary(warner)
...
## -----------------------------------------------------------------------------
RRcor(x=data.W$response, y=data.W$cov, models=c("Warner", "direct"), 
      p.list=list(.2), bs.n=0, bs.type = c("se.n", "se.p", "pval"), nCPU=1)

## -----------------------------------------------------------------------------
cor(x=data.W$true, y=data.W$cov)

## -----------------------------------------------------------------------------
log1 <- RRlog(formula=response~cov, data=data.W, 
              model="Warner", p=.2, LR.test=TRUE, fit.n = 1)
summary(log1)
...
mixmod <- RRmixed(response ~ cov + (1 | cluster), 
                  data=data.W, model = "Warner", p = .2)
mixmod

## -----------------------------------------------------------------------------
lin1 <- RRlin(formula=cov~response, data=data.W, 
              models="Warner", p.list=.2, bs.n=0, fit.n=1)
summary(lin1)

## -----------------------------------------------------------------------------
data.W$pred <- rnorm(1000)
lin2 <- RRlin(formula=cov~response + pred, data=data.W, 
              models="Warner", p.list=list(.2), bs.n=0, fit.n=1)
summary(lin2)
...
z.age <- scale(c(rnorm(500, mean=RR$true, sd=3), 
                 rnorm(500, mean=.5, sd=3)))         
# fit full model 
# (testing difference in prevalence estimates and interaction)
fit <- RRlog(response ~ RR.design * z.age, model="Warner", 
...
logmod <- RRlog(RR1$response ~ cov, model="SLD", p=c(.2,.8), 
                group=RR1$group, fit.n = 1)
summary(logmod)
...
RRcor( responses, models=c("SLD","SLD","direct"), 
       p.list=list(c(.2,.8), c(.3, .7)), group=group)

# linear model with 2 RR predictors
linmod <- RRlin(cov ~ RR1$response+RR2$response, models=c("SLD","SLD"), 
                p.list=list(c(.2,.8), c(.3, .7)), group=group, fit.n=1)
summary(linmod)
