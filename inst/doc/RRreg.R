...
## ------------------------------------------------------------------------
log1 <- RRlog(formula=response~cov, data=data.W, model="Warner", p=.2, LR.test=TRUE, fit.n = c(1,5))
summary(log1)
...
fit <- RRlog(response ~ group * z.age, model="Warner", p=.35, group=group, LR.test=TRUE, fit.n = c(1,5)) 
summary(fit)
...
logmod <- RRlog(RR1$response ~ cov, model="SLD", p=c(.2,.8), group=RR1$group, fit.n = c(1,5))
summary(logmod)
