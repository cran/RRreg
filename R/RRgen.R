...
#' @param model specifying a RR model, see \code{\link{RRuni}} for details about RR models 
...
#' @param groupRatio only required for multiple group models, e.g., \code{SLD} and \code{CDM}
...
#'         
#' @seealso \code{\link{RRuni}} for a detailed description of the models and \code{\link{RRlog}} and \code{\link{RRcor}} for the multivariate analysis of RR data
#' @examples 
...
RRgen <- function(n,pi.true,
                  model = c("Warner","UQTknown","UQTunknown","Mangat","Kuk","FR","Crosswise","CDM","CDMsym","SLD"), p, 
...
#                warning(paste0("The prevalence of the unrelated question was randomly set to ", round(pi.true[2],3),". To explicitly choose the prevalence, set 'pi.true=c(pi.sensitive, pi.unrelated)'"),call.=F)
             ...
             randUQ <- runif(n)
             split <- c(rep(1,floor(n*groupRatio)),rep(2,n-floor(n*groupRatio)))
